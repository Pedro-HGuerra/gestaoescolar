...
def reports(opcao_relatorio:int=0):
    if opcao_relatorio == 1:
        ...
        relatorio.get_relatorio_notaAlunoTrabalho()
...
def inserir(opcao_inserir:int=0):
    if opcao_inserir == 1:                               
        ...
        while config.continuaProcedimento(continua):
            relatorio.get_relatorio_prova()
            novo_prova = ctrl_prova.inserir_prova()
            continua = input("Deseja continuar? (S = Sim / N = Não)").upper()
...
def atualizar(opcao_atualizar:int=0):
    if opcao_atualizar == 1:
        ...
        while config.continuaProcedimento(continua):
            relatorio.get_relatorio_prova()
            prova_atualizado = ctrl_prova.atualizar_prova()
            continua = input("Deseja continuar? (S = Sim / N = Não)").upper()
...
def excluir(opcao_excluir:int=0):
    ...
    if opcao_excluir == 1:
        ...
        while config.continuaProcedimento(continua):
            relatorio.get_relatorio_trabalho()
            novo_trabalho = ctrl_trabalho.excluir_trabalho()
            continua = input("Deseja continuar? (S = Sim / N = Não)").upper()
